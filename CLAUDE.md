...
- **⚠️ `persona.env.SYSTEM_PROMPT` es SPAWN-BAKED** (`spawnVm` L1096, va al env del microVM), NO se lee per-turno. Editarlo (DB o `set_agent_prompt`) aplica solo a VMs **nuevas/recicladas** — la caja viva/suspendida sigue con el prompt viejo hasta que el reaper la recicle. Para que un cambio de persona surta efecto YA hay que **reciclar la caja** (`DELETE /v1/sandbox/:id` → el host detacha la ruta `agentId=null` → próximo mensaje cold-spawnea). Mismo patrón que `ANTHROPIC_MODEL` (sección Modelo). Backup del prompt original antes de cualquier write directo a DB.
...
- 🚨 **BUG URGENTE ABIERTO (2026-08-01): el desalojo es por-AGENTE, el techo es por-CUENTA.** `napping` solo busca `status:"suspended"` **del mismo `fleetAgentId`** (~L1397), pero el gate cuenta TODAS las sandboxes del owner. → un FleetAgent **nuevo** (sin cajas propias) relanza `FleetAgentAtCapacity` aunque sus hermanos tengan cajas dormidas: **una cuenta en su techo no puede estrenar un agente**. Reproducido en prod (`account at sandbox budget (2/2)` en el primer turno de un agente recién creado). NO es fix de una línea: hoy el código **adopta** la VM liberada, y una VM trae el `workerTemplate` y el env horneado de SU agente (persona, prompt, credencial del motor) → una caja `claude-worker` no sirve para un `codex-worker`. El fix correcto es **destruir** la caja ajena y spawnear la propia (cold ~12s), no adoptarla. Workaround aplicado: subir de plan. Ver memoria `todo_fleet_eviction_cross_agent_urgent`.
