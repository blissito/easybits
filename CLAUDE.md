...
- **render** (`fleetRender.ts`): la caja `render-svc` (:9300) es un **servidor Chromium propio** (`localhost/render-svc:latest`, 2048 MB) que habla **JSON**: `POST /render/pdf` y `POST /render/screenshot` con `{html, viewport, pdf|screenshot}` (opciones de `page.pdf()`/`page.screenshot()`) y devuelve los bytes + header `x-broken-images`. `renderClient.renderOnBox(ctx,mode,payload)` es el ÚNICO sitio que habla ese contrato; `fleetRender.renderViaBoxAndStore` lo envuelve y sube a Files → `{fileId,url,broken}`.
  ...
  - ⚠️ **Hornear el template en UN fierro NO lo despliega**: el router coloca por RAM libre, así que la caja puede nacer en **box-b**, que NO tiene docker ni `build_template.sh`. Hay que copiarle el `.ext4` desde A (comprimido, vía la Mac: A y B no tienen confianza SSH) y **re-vincular el loop en AMBOS** (`losetup -a | grep '(deleted)'` → destruir las cajas que lo retienen → `losetup -d` → `losetup -r`). Sin el re-bind, las VMs nuevas siguen con el código VIEJO aunque el ext4 esté correcto. Pista de que la caja está en el otro fierro: cero entradas de ese template en el `store.json` de A. Ver memoria `feedback_bake_template_every_fierro`.
