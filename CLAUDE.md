...
- **⚠️ BUG ABIERTO (reportado 2026-07-09): cajas de servicio se levantan MÚLTIPLES en vez de reusarse.** Contra el diseño ("UNA caja por `(ownerId,kind)`"), en la práctica `render` (y otras cajas de servicio) spawnea instancias duplicadas para el mismo owner en vez de reusar la existente → consume budget de sandboxes de más y explica el conteo raro en `/dash/flota` (varias cajas `render`/`render·dormida`). Probable causa: race en `ensureServiceBox` (dos turnos concurrentes del mismo owner ven "no running" antes de que el primer `spawnServiceBox` registre la caja → ambos spawnean), o la llave `(ownerId,kind)` no está deduplicando (spawn por API cruda del host que salta `ensureServiceBox`, o el registro de la caja no es atómico). FIX pendiente: lock/dedup por `(ownerId,kind)` alrededor del spawn (reserva atómica antes de spawnear, como el `reserveVm` de los workers), y barrer duplicados existentes. Relacionado con "NO crear cajas de más" (sección Capacidad).
