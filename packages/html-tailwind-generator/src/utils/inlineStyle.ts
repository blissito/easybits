 ...
 *
 * The document design guideline is: ONLY Tailwind classes, NEVER inline styles —
 * the only allowed exception is `font-family`. When an element carries any other
 * inline declaration it wins by specificity and the toolbar's utility edits
 * (color/size/spacing) silently do nothing. We don't transform such HTML
 * (lossy/fragile); we just flag it so the user understands why.
 */
