 ...
 *
 * Run (all pools):      npx tsx scripts/update-fleet-agent-reap-timings.ts
 * Run (single fleetAgent):    npx tsx scripts/update-fleet-agent-reap-timings.ts <fleetAgentId>
 *
...
async function main() {
  const fleetAgentId = process.argv[2];
  ...
  const pools = fleetAgentId
    ? await db.fleetAgent.findMany({ where: { id: fleetAgentId } })
    : await db.fleetAgent.findMany();
  if (!pools.length) throw new Error(fleetAgentId ? `fleetAgent ${fleetAgentId} not found` : "no pools");
  ...
  for (const p of pools) {
    ...
    await db.fleetAgent.update({ where: { id: p.id }, data });
    console.log(
