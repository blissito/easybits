// One-off cleanup: move the raw Brightdata token that was hacked into a fleetAgent's
...
const FLEET_AGENT_ID = "6a3c046d2cf7d7f4163f739d";
...
async function main() {
  ...
  if (!fleetAgent) {
    console.log(`fleetAgent ${FLEET_AGENT_ID} not found — nothing to do`);
    return;
  }
  const catalog = (fleetAgent.mcpCatalog as Array<{ name: string; env?: Record<string, string> }> | null) ?? [];
  const bd = catalog.find((e) => e.name === "brightdata");
  const rawToken = bd?.env?.API_TOKEN;

  console.log(`fleetAgent ${FLEET_AGENT_ID} owner ${fleetAgent.ownerId}`);
  ...
  const existing = await getSecretValue(fleetAgent.ownerId, SECRET_NAME).catch(() => null);
  console.log(`  secret ${SECRET_NAME} already in vault: ${existing ? "yes" : "no"}`);

  if (!apply) {
    console.log("\nDRY-RUN. Would:");
    if (rawToken && !existing) console.log(`  • createSecret(${fleetAgent.ownerId}, ${SECRET_NAME}, <token>)`);
    console.log(`  • set fleetAgent.mcpCatalog = null (drop hack + stale denik builtin; builtins/curated come from code)`);
  ...
  if (rawToken && !existing) {
    await createSecret(fleetAgent.ownerId, { name: SECRET_NAME, value: rawToken });
  ...
  await db.fleetAgent.update({ where: { id: FLEET_AGENT_ID }, data: { mcpCatalog: [] } });
  console.log(`  ✓ fleetAgent.mcpCatalog cleared (now [] — curated/builtins from code)`);
  console.log("done.");
}
