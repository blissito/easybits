...
export default [
  ...
  route("/aprende/:curso.md", "routes/aprende.md.ts", { id: "aprende-md-course" }),
  route("/aprende/:curso/:leccion.md", "routes/aprende.md.ts", { id: "aprende-md-lesson" }),
  route("/aprende", "routes/aprende.tsx"),
  route("/aprende/:curso", "routes/aprende.$curso.tsx"),
  route("/aprende/:curso/certificado", "routes/aprende.$curso.certificado.tsx"),
  route("/aprende/:curso/certificado/:token", "routes/aprende.$curso.certificado.$token.tsx"),
  route("/aprende/:curso/:leccion", "routes/aprende.$curso.$leccion.tsx"),
  ...
  ...prefix("dash", [
    ...
    layout("components/DashLayout/DashLayout.tsx", [
      ...
      ...prefix("video-projects", [
        index("routes/dash/video-projects/list.tsx"),
        route(":id", "routes/dash/video-projects/detail.tsx"),
      ]),
      ...
      route("forms/:id", "routes/dash/forms.$id.tsx"),
      ...
      // Hosting: el panel del DUEÑO de un sitio (su URL, dominios, versiones,
      // variables y logs). Distinto de /dash/flota, que es el HUD de capacidad
      // de los agentes: ahí una caja de hosting salía como un cuadrito con el
      // nombre de su template y nada más, así que quien tenía un sitio no
  ...
  route("api/cron/check-hosting", "routes/api/cron/check-hosting.ts"),
  ...
  ...prefix("api/v2", [
    ...
    route("calls/transcript", "routes/api/v2/calls.transcript.ts"),
    ...
    route("calls/:id/transcript", "routes/api/v2/calls.$id.transcript.ts"),
    ...
    route("sandboxes/:id/git/:op", "routes/api/v2/sandbox-git.ts"),
    ...
    route("video-projects", "routes/api/v2/video-projects.ts"),
    route("video-projects/:id", "routes/api/v2/video-project.ts"),
    route("video-projects/:id/scenes", "routes/api/v2/video-project-scenes.ts"),
    route("video-projects/:id/scenes/:sceneId", "routes/api/v2/video-project-scene.ts"),
    route("video-projects/:id/audio", "routes/api/v2/video-project-audio.ts"),
    ...
    route("forms", "routes/api/v2/forms.ts"),
    route("forms/:formId", "routes/api/v2/forms.$formId.ts"),
    ...
    route("forms/:formId/submissions", "routes/api/v2/forms.$formId.submissions.ts"),
