/**
 * Simple document editor (NO real-time collab). Same token-auth surface as
 * /collab/document/:token but mounts the instant, server-less editor: it seeds from
 * Landing.sections on mount (no Hocuspocus box, no WS wait) so the ARTIFACT in Teams
 * shows the freshly-created doc immediately. Embeddable (?embed=1 + allowedOrigins).
 *
 * Co-editing (live cursors) is a SHARE action → that surface stays at
 * /collab/document/:token, spun up on demand from the editor's "Compartir" button.
 */
import { redirect } from "react-router";
import type { Route } from "./+types/document.$token";
import { verifyShareToken, buildShareCookie, type SharePermission } from "~/.server/shareLinks";
import { db } from "~/.server/db";
import type { Section3 } from "~/lib/landing3/types";
import SimpleDocumentEditor from "~/components/share/SimpleDocumentEditor";
...
export const loader = async ({ params, request }: Route.LoaderArgs) => {
  ...
  return new Response(
    JSON.stringify({ landingId: landing.id, sections, token }),
    { headers: { ...Object.fromEntries(headers), "Content-Type": "application/json" } },
  );
};

export default function SimpleDocumentRoute({ loaderData }: Route.ComponentProps) {
  const data = loaderData as { landingId: string; sections: Section3[]; token: string };
  return <SimpleDocumentEditor {...data} />;
}
