...
await cfg({ action: "set-prompt", groupId: "mi-app", systemPrompt: "Aquí hablas con clientes finales: no menciones precios internos." });`,
          ...
          <section id="eve" className="mb-16">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              <a href="https://eve.dev" className="underline">eve</a> es el framework open-source de Vercel para agentes: un agente es un directorio y cada sesión es un run durable. Cuando un agente necesita ejecutar código, eve le pide una caja a un <strong>provider de sandbox</strong>. <code className="bg-gray-100 px-1 rounded">@easybits.cloud/eve-sandbox</code> es ese provider: cada sesión de agente corre en su propia microVM, en tu cuenta. Requiere Node ≥ 24 y eve ≥ 0.64 (para eve ≤ 0.63 fija <code className="bg-gray-100 px-1 rounded">@easybits.cloud/eve-sandbox@0.1</code>). <strong>Ruta gratis</strong>: tu servidor eve se queda donde está (Vercel o tu laptop con <code className="bg-gray-100 px-1 rounded">eve dev</code>/<code className="bg-gray-100 px-1 rounded">eve start</code>) y sólo las sesiones corren en EasyBits; cabe en Byte (una conversación a la vez, sesiones de 1 h). <strong>Ruta hospedada</strong>: el servidor eve también en una caja (<code className="bg-gray-100 px-1 rounded">eve-nitro</code>) + una caja por sesión = 2 cajas, desde Mega ($499 MXN/mes); <code className="bg-gray-100 px-1 rounded">eve-world</code> sólo aplica ahí.
            </p>
...
// Se prepara una vez en \`eve build\` (lee EASYBITS_API_KEY) y se captura como plantilla.
...
export default defineSandbox(() => environment.open());`} />
            <p className="text-gray-600 mb-4 text-sm">
              <code className="bg-gray-100 px-1 rounded">eve build</code> llama a <code className="bg-gray-100 px-1 rounded">prepare()</code>: caja temporal <code className="bg-gray-100 px-1 rounded">node</code> + <code className="bg-gray-100 px-1 rounded">agent/sandbox/workspace/</code> en <code className="bg-gray-100 px-1 rounded">/workspace</code> + skills en <code className="bg-gray-100 px-1 rounded">~/.agents/skills</code> + tu <code className="bg-gray-100 px-1 rounded">prepare(sandbox)</code> → <strong>plantilla derivada</strong> (<code className="bg-gray-100 px-1 rounded">template-snapshot</code>, clave de contenido, 12-24 MB, se borra sola a los 30 días sin uso; medido: 9.7 s la primera vez, 0.3 s reusada). <code className="bg-gray-100 px-1 rounded">environment.open()</code> → <code className="bg-gray-100 px-1 rounded">start()</code>: la caja nace de esa plantilla con todo hecho (~5 s) y entre turnos duerme y despierta en ~1 s; si el servidor reinicia, <code className="bg-gray-100 px-1 rounded">resume()</code> la reabre, y si ya no existe falla (contrato de eve) y eve abre una sesión nueva. Egress <strong>por caja</strong>: <code className="bg-gray-100 px-1 rounded">environment.open({"{ networkPolicy }"})</code> o <code className="bg-gray-100 px-1 rounded">sandbox.setNetworkPolicy()</code> en caliente (<code className="bg-gray-100 px-1 rounded">allow-all</code>, <code className="bg-gray-100 px-1 rounded">deny-all</code> o allow-list por dominio; <code className="bg-gray-100 px-1 rounded">transform</code> no está soportado). Modelo: el scaffold trae un string del AI Gateway de Vercel; fuera de Vercel usa <code className="bg-gray-100 px-1 rounded">anthropic()</code> de <code className="bg-gray-100 px-1 rounded">eve/models/anthropic</code> (lee <code className="bg-gray-100 px-1 rounded">ANTHROPIC_API_KEY</code>) u <code className="bg-gray-100 px-1 rounded">openai()</code>. En local <code className="bg-gray-100 px-1 rounded">eve start</code> responde 401 aun desde localhost: usa <code className="bg-gray-100 px-1 rounded">eve dev --no-ui</code>. Para hospedar el <strong>servidor</strong> eve usa el template <code className="bg-gray-100 px-1 rounded">eve-nitro</code> (Node 24, pnpm, eve CLI 0.65, <code className="bg-gray-100 px-1 rounded">/data</code> persistente —no es el cwd: <code className="bg-gray-100 px-1 rounded">cd /data</code> explícito—, puerto 3000), espera <code className="bg-gray-100 px-1 rounded">status=running</code> antes de <code className="bg-gray-100 px-1 rounded">exec</code> (sobre una caja <code className="bg-gray-100 px-1 rounded">starting</code> responde <code className="bg-gray-100 px-1 rounded">409 SandboxNotReady</code>), instala con <code className="bg-gray-100 px-1 rounded">pnpm add … --allow-build=cbor-extract</code> (pnpm 12 bloquea ese build script), sube los archivos del agente con <code className="bg-gray-100 px-1 rounded">POST /sandboxes/:id/files/write</code>, corre <code className="bg-gray-100 px-1 rounded">npx eve build</code> con <code className="bg-gray-100 px-1 rounded">EASYBITS_API_KEY</code> en el env y expón el 3000; en producción eve exige auth en su API HTTP (edita <code className="bg-gray-100 px-1 rounded">agent/channels/eve.ts</code>, p. ej. <code className="bg-gray-100 px-1 rounded">httpBasic</code>). El estado durable (runs, hooks, streams) puede vivir en EasyBits DB con <code className="bg-gray-100 px-1 rounded">@easybits.cloud/eve-world</code> (la caja nace con <code className="bg-gray-100 px-1 rounded">EASYBITS_DB_URL</code> en su entorno, sin token; para retomar en una caja nueva, créala con el mismo valor, que está en <code className="bg-gray-100 px-1 rounded">metadata.eve_db_url</code>). Landing: <a href="/eve" className="underline">/eve</a> · guía completa: <a href="/docs/eve.md" className="underline">/docs/eve.md</a> · skill <code className="bg-gray-100 px-1 rounded">easybits-eve</code>.
            </p>
          </section>
