...
export default defineSandbox({
  ...
  async bootstrap({ use }) {
    ...
    await s.run({ command: "git clone https://github.com/tu-org/herramientas /workspace/tools && cd /workspace/tools && npm ci" });
            ...
            <p className="text-gray-600 mb-4 text-sm">
              <code className="bg-gray-100 px-1 rounded">prewarm</code> se convierte en un snapshot copy-on-write (<code className="bg-gray-100 px-1 rounded">eve:&lt;templateKey&gt;</code>) que se reusa en cada build; cada sesión es un fork de ese snapshot (~7 s) que entre turnos duerme con <code className="bg-gray-100 px-1 rounded">sandbox_set_idle</code> y despierta en ~1 s. <code className="bg-gray-100 px-1 rounded">setNetworkPolicy</code> fija el egress <strong>por caja</strong> (<code className="bg-gray-100 px-1 rounded">allow-all</code>, <code className="bg-gray-100 px-1 rounded">deny-all</code> o allow-list por dominio; <code className="bg-gray-100 px-1 rounded">transform</code> no está soportado). Para hospedar el <strong>servidor</strong> eve usa el template <code className="bg-gray-100 px-1 rounded">eve-nitro</code> (Node 24, pnpm, eve CLI, <code className="bg-gray-100 px-1 rounded">/data</code> persistente, puerto 3000) y expón el 3000. El estado durable (runs, hooks, streams) puede vivir en EasyBits DB con <code className="bg-gray-100 px-1 rounded">@easybits.cloud/eve-world</code> (la caja eve-nitro nace con <code className="bg-gray-100 px-1 rounded">EASYBITS_DB_URL</code>, sin token). Landing: <a href="/eve" className="underline">/eve</a> · guía completa: <a href="/docs/eve.md" className="underline">/docs/eve.md</a> · skill <code className="bg-gray-100 px-1 rounded">easybits-eve</code>.
            </p>
          </section>
