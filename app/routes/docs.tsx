...
await cfg({ action: "set-prompt", groupId: "mi-app", systemPrompt: "Aquí hablas con clientes finales: no menciones precios internos." });`,
          ...
          <section id="eve" className="mb-16">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              <a href="https://eve.dev" className="underline">eve</a> es el framework open-source de Vercel para agentes: un agente es un directorio y cada sesión es un run durable. Cuando un agente necesita ejecutar código, eve le pide una caja a un <strong>SandboxBackend</strong>. <code className="bg-gray-100 px-1 rounded">@easybits.cloud/eve-sandbox</code> es ese backend: cada sesión de agente corre en su propia microVM, en tu cuenta. Requiere Node ≥ 24. <strong>Ruta gratis</strong>: tu servidor eve se queda donde está (Vercel o tu laptop con <code className="bg-gray-100 px-1 rounded">eve dev</code>/<code className="bg-gray-100 px-1 rounded">eve start</code>) y sólo las sesiones corren en EasyBits; cabe en Byte (una conversación a la vez, sesiones de 1 h). <strong>Ruta hospedada</strong>: el servidor eve también en una caja (<code className="bg-gray-100 px-1 rounded">eve-nitro</code>) + una caja por sesión = 2 cajas, desde Mega ($499 MXN/mes); <code className="bg-gray-100 px-1 rounded">eve-world</code> sólo aplica ahí.
            </p>
...
export default defineSandbox({
  ...
  async bootstrap({ use }) {
    ...
    await s.run({ command: "git clone https://github.com/tu-org/herramientas /workspace/tools && cd /workspace/tools && npm ci" });
            ...
            <p className="text-gray-600 mb-4 text-sm">
              <code className="bg-gray-100 px-1 rounded">prewarm</code> se convierte en una <strong>plantilla derivada</strong> (<code className="bg-gray-100 px-1 rounded">template-snapshot</code>, clave <code className="bg-gray-100 px-1 rounded">eve:&lt;templateKey&gt;</code> + hash de las opciones, 12-22 MB, se borra sola a los 30 días sin uso) que se captura en el primer <code className="bg-gray-100 px-1 rounded">eve start</code> o <code className="bg-gray-100 px-1 rounded">eve dev</code> (no en <code className="bg-gray-100 px-1 rounded">eve build</code>, que compila y valida el backend — por eso exige <code className="bg-gray-100 px-1 rounded">EASYBITS_API_KEY</code> y es obligatorio antes de arrancar; medido: 8.6 s la primera vez, 0.3 s reusada). En local <code className="bg-gray-100 px-1 rounded">eve start</code> responde 401 aun desde localhost: la sesión local entra con <code className="bg-gray-100 px-1 rounded">eve dev --no-ui</code> y se reusa en los arranques siguientes; cada sesión nace de esa plantilla con el bootstrap ya hecho (~4 s) y entre turnos duerme con <code className="bg-gray-100 px-1 rounded">sandbox_set_idle</code> y despierta en ~1 s. <code className="bg-gray-100 px-1 rounded">setNetworkPolicy</code> fija el egress <strong>por caja</strong> (<code className="bg-gray-100 px-1 rounded">allow-all</code>, <code className="bg-gray-100 px-1 rounded">deny-all</code> o allow-list por dominio; <code className="bg-gray-100 px-1 rounded">transform</code> no está soportado). Para hospedar el <strong>servidor</strong> eve usa el template <code className="bg-gray-100 px-1 rounded">eve-nitro</code> (Node 24, pnpm, eve CLI 0.58.1, <code className="bg-gray-100 px-1 rounded">/data</code> persistente —no es el cwd: <code className="bg-gray-100 px-1 rounded">cd /data</code> explícito—, puerto 3000), espera <code className="bg-gray-100 px-1 rounded">status=running</code> antes de <code className="bg-gray-100 px-1 rounded">exec</code> (sobre una caja <code className="bg-gray-100 px-1 rounded">starting</code> responde <code className="bg-gray-100 px-1 rounded">409 SandboxNotReady</code>), instala con <code className="bg-gray-100 px-1 rounded">pnpm add … --allow-build=cbor-extract</code> (pnpm 12 bloquea ese build script), sube los archivos del agente con <code className="bg-gray-100 px-1 rounded">POST /sandboxes/:id/files/write</code> y expón el 3000; en producción eve exige auth en su API HTTP (edita <code className="bg-gray-100 px-1 rounded">agent/channels/eve.ts</code>, p. ej. <code className="bg-gray-100 px-1 rounded">httpBasic</code>) y el modelo va como objeto de un proveedor directo del AI SDK (<code className="bg-gray-100 px-1 rounded">anthropic("claude-sonnet-5")</code> con <code className="bg-gray-100 px-1 rounded">@ai-sdk/anthropic</code>), no como el string del AI Gateway de Vercel que trae el scaffold (<code className="bg-gray-100 px-1 rounded">"openai/gpt-5.6-luna-fast"</code>). El estado durable (runs, hooks, streams) puede vivir en EasyBits DB con <code className="bg-gray-100 px-1 rounded">@easybits.cloud/eve-world</code> (la caja nace con <code className="bg-gray-100 px-1 rounded">EASYBITS_DB_URL</code> en su entorno, sin token; sólo se pasa a mano en el env del <code className="bg-gray-100 px-1 rounded">/bg</code> en una caja de fork/snapshot o para retomar runs en una caja nueva, con el mismo valor de <code className="bg-gray-100 px-1 rounded">metadata.eve_db_url</code>). Landing: <a href="/eve" className="underline">/eve</a> · guía completa: <a href="/docs/eve.md" className="underline">/docs/eve.md</a> · skill <code className="bg-gray-100 px-1 rounded">easybits-eve</code>.
            </p>
          </section>
