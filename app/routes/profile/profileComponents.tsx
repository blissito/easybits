...
// Read-only list of the user's active fleetAgent-box reservations (Stripe subscriptions).
...
export const AddonsCard = ({ reservations = [] }: { reservations?: any[] }) => {
  ...
  const totalBoxes = Math.ceil(totalAgents / FLEET_BOX.agents);
  return (
    <section className="border-2 bg-white max-w-2xl border-black rounded-2xl mt-8">
      <div className="p-4 md:p-6 flex flex-col items-start">
        <div className="flex flex-wrap w-full justify-between mb-3 gap-3 md:flex-nowrap md:gap-0">
          <h3 className="font-semibold text-xl md:text-2xl">Cajas de fleetAgent</h3>
          <span className="text-xl font-semibold md:text-2xl">
            ${totalBoxes * FLEET_BOX.priceMxn} mxn/mes
          </span>
        </div>
        <ul className="w-full flex flex-col gap-2">
          {reservations.map((r) => {
            const boxes = Math.max(1, Math.ceil((r.agents || 0) / FLEET_BOX.agents));
            return (
              <li
                key={r.id}
              ...
              >
                <span className="font-semibold">
                  {boxes === 1 ? "Caja del fleetAgent" : `${boxes} cajas del fleetAgent`}
                  <span className="text-iron font-normal"> · {r.agents} agentes</span>
                </span>
                <span className="font-semibold">${boxes * FLEET_BOX.priceMxn} mxn/mes</span>
              </li>
            );
          })}
        </ul>
