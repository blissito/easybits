...
export const loader = async ({ request }: Route.LoaderArgs) => {
  ...
  const landings = await db.landing.findMany({
    where: { ownerId: user.id, version: 3 },
  ...
  const items = landings.map((l) => {
    ...
    return {
      ...
      firstLabel: (sections[0] as any)?.label || "",
...
export default function Landings3List() {
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            HTML libre + edici&oacute;n visual &middot; {items.length}{" "}
            {items.length === 1 ? "landing" : "landings"}
          </p>
        </div>
        <BrutalButton onClick={() => navigate("/dash/landings3/new")}>
          + Nueva
        </BrutalButton>
      </div>
      ...
      {items.length === 0 ? (
        ...
        >
          ...
          <p className="text-lg font-bold text-gray-700">
            No tienes landings v3 a&uacute;n
          </p>
          <p className="text-sm text-gray-500 mt-1 mb-6">
            AI genera HTML+Tailwind con libertad creativa total
          </p>
          <BrutalButton onClick={() => navigate("/dash/landings3/new")}>
            Crear landing
          </BrutalButton>
          ...
          <AnimatePresence mode="popLayout">
            {items.map((item, i) => (
              ...
              >
                ...
                >
                  <div className="h-3 w-full bg-gradient-to-r from-brand-500 to-purple-500" />
                  <div className="p-5">
