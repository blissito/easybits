...
export const loader = async ({ request }: Route.LoaderArgs) => {
  ...
  const landings = await db.landing.findMany({
    where: { ownerId: user.id, version: 5 },
...
export default function Landings4List() {
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Editor visual drag &amp; drop &middot; {items.length}{" "}
            {items.length === 1 ? "landing" : "landings"}
          </p>
        </div>
        <BrutalButton onClick={() => navigate("/dash/landings4/new")}>
          + Nueva
        </BrutalButton>
      </div>
      ...
      {items.length === 0 ? (
        ...
        >
          ...
          <p className="text-lg font-bold text-gray-700">
            No tienes landings v4 a&uacute;n
          </p>
          <p className="text-sm text-gray-500 mt-1 mb-6">
            Drag &amp; drop visual con bloques predise&ntilde;ados
          </p>
          <BrutalButton onClick={() => navigate("/dash/landings4/new")}>
            Crear landing
          </BrutalButton>
          ...
          <AnimatePresence mode="popLayout">
            {items.map((item, i) => (
              ...
              >
                ...
                >
                  <div className="h-3 w-full bg-gradient-to-r from-emerald-500 to-cyan-500" />
                  <div className="p-5">
