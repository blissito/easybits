...
```js
await sbx.execBackground("exec python3 -m http.server 3000");
const { url } = await sbx.exposePort(3000);
console.log(url); // https://sb-...-3000.sandboxes.easybits.cloud
```

Ese `exec` de más no es adorno: hace que el shell sea **reemplazado** por tu proceso en vez de quedarse como su padre, así que cuando lo mates la señal le llega al servidor de verdad y el puerto queda libre.
...
// ¿tu agente perdió el execId? const { processes } = await sbx.bgList();
```
