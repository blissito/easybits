...
// ...y si lo perdiste: const { processes } = await sbx.bgList();
```
...
`bgKill` señala al **grupo de procesos** completo: primero un SIGTERM para que lo que corre alcance a cerrar, y un SIGKILL después de la gracia. Importa cuando tu comando forkea —un `npm test` que levanta un server de pruebas, por ejemplo—, porque matar solo al shell dejaría a ese hijo vivo ocupando el puerto. Matar algo que ya terminó no es un error: te responde con `alreadyExited`, así que puedes llamarlo sin preguntar primero.
