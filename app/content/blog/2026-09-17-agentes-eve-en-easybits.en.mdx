...
Four calls: create the box, create the app inside it with `eve init` (or clone yours) and install both packages, start the server, expose the port. First the base URL and headers:
...
`EASYBITS_DB_URL` is already in the box's environment; no need to pass it in `env`. The `eve-nitro` template ships Node 24, pnpm and the eve CLI; `/data` is persistent and the public URL proxies `/eve/` and `/.well-known/workflow/` with no configuration. The server runs there; every agent it serves gets its own microVM through the backend above.
...
An `eve-nitro` box is born with `EASYBITS_DB_URL` already set: no token to paste. We measured it in production with an 8-step run: we destroyed the box halfway through step 2; a fresh box on the same database started, resumed at step 3 without repeating the earlier ones — 59 seconds after the first box died — and finished the run. It is a port of the official Postgres world to libSQL, on the same version line as eve.
