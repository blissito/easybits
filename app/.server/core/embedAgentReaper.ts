// Embed-agent idle reaper — standalone claude-worker agents (no fleetAgent).
//
// denik corre 1 agente claude-worker por cliente bajo UNA cuenta de EasyBits
// (fleetAgentId=null; no son workers del fleetAgent). Para que la flota escale a N agentes
...
// memoria como en el fleetAgent.
//
// Vive en su propio módulo (no dentro de fleetAgentOperations.ts) para no chocar con
// el trabajo del fleetAgent. Tiene su PROPIO busy-set: los ids de agentes embed son
// disjuntos de los de los workers del fleetAgent, y este reaper solo toca agentes
// fleetAgentId=null, así que no hay traslape.
...
// (mismo caveat que el busyVms del fleetAgent); mover a marca en DB antes de escalar
...
export async function reapIdleEmbedAgents(): Promise<{ suspended: number }> {
  ...
  // Standalone claude-worker agents (fleetAgentId=null) running and idle past cutoff.
