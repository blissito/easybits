...
- Never embed \`putUrl\` (it's a write-only signed URL). Never embed a URL containing \`/mcp/\` or \`signed=\` (always private).`,
  ...
  server.tool(
    "agent_create",
    "Spawn a long-lived agent inside a Firecracker microVM and return its reachable agentUrl. Distinct from `agent_run` (Claude one-shot managed): this returns a persistent endpoint you can post messages to. Use template='chat-openai' or 'chat-anthropic' for embeddable chat with SSE streaming, or 'goose'/'claude-code'/etc for coding harness. The 'desktop-ghosty'/'computer-ghosty'/'computer-ghosty-gemini' templates boot a real Linux XFCE desktop — after status becomes 'running', `agent_list` returns a public `desktopUrl` (noVNC, openable in any browser with no login: the unguessable subdomain is the capability) and, for the computer-* ones, a `terminalUrl`. The `env` map (API keys, model, system prompt) is injected into the runtime at start time — never baked into the template image. Returns { sandboxId, agentUrl, healthUrl }.",
    {
      template: z.enum(["chat-openai", "chat-anthropic", "goose", "claude-code", "ghostyclaw", "openclaw", "desktop-ghosty", "computer-ghosty", "computer-ghosty-gemini"]).describe("Agent template. 'chat-*' = persistent Express+SSE runtime; 'ghostyclaw' = always-on nanoclaw daemon (WhatsApp/Slack/Telegram); 'openclaw' = OpenClaw personal AI; 'desktop-ghosty' = live noVNC desktop; 'computer-ghosty'/'computer-ghosty-gemini' = computer-use workstation (desktop the agent drives + terminal); others = harness/autonomous."),
