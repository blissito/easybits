...
- Never embed \`putUrl\` (it's a write-only signed URL). Never embed a URL containing \`/mcp/\` or \`signed=\` (always private).`,
  ...
  server.tool(
    ...
    "Start a managed Claude agent (Claude Agent SDK loop) inside a fresh Firecracker microVM. By default the agent has Bash/Read/Write/Edit/Glob/Grep/WebFetch (interactive/session tools like Agent, AskUserQuestion, Skill, TodoWrite are denied — this is a headless VM) and full root with open internet egress. EasyBits supplies the Anthropic API key by default and bills the run; pass `secrets: [\"ANTHROPIC_API_KEY\", ...]` after registering it via `secret_set` to switch to BYOK (Bring Your Own Key) mode — Claude tokens are then billed by Anthropic directly to your account, EasyBits charges 0 for tokens. Pass other entries in `secrets: [...]` to inject EasyBits Secrets as env vars in the sandbox — spawned MCP children (brightdata, easybits, etc.) inherit them via process.env. ASYNC: returns { jobId, status:'running' } immediately. Poll with `agent_run_status({ jobId })` until status is 'done'/'error'/'expired'. Sandbox auto-destroys at 30 min TTL. Default: claude-sonnet-4-6, 30 turns max.",
    {
      ...
      max_turns: z.number().int().min(1).optional().describe("Max agent loop iterations before forced stop (default 30). The 30-min sandbox TTL caps total runtime regardless. With BYOK, cost is on your Anthropic account."),
