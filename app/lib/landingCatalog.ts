...
export function renderSection(section: LandingSection, theme: string): string {
  const t = getThemeVars(theme);
  const renderer = RENDERERS[section.type];
  if (!renderer) return `<!-- unknown section type: ${section.type} -->`;
  return renderer(section.props, t);
}

type ThemeVars = ReturnType<typeof getThemeVars>;
type Renderer = (props: Record<string, any>, t: ThemeVars) => string;
...
const RENDERERS: Record<SectionType, Renderer> = {
  hero: (p, t) => {
    ...
    return `<section style="background:${t.bg};color:${t.text}" class="relative overflow-hidden">
  <div class="max-w-7xl mx-auto px-6 py-24 lg:py-32 flex flex-col lg:flex-row items-center gap-12">
    <div class="flex-1 text-center lg:text-left">
      ...
      <div class="mt-10">
        <a href="${ctaUrl}" style="background:${t.accent};color:${t.bg}" class="inline-block px-8 py-4 rounded-lg text-lg font-bold shadow-lg hover:opacity-90 transition">${ctaText}</a>
      </div>
    </div>
  ...
  logoCloud: (p, t) => {
    ...
    return `<section style="background:${t.bg};color:${t.text}" class="py-16">
  ...
  features: (p, t) => {
    ...
    return `<section style="background:${t.bg};color:${t.text}" class="py-20">
  ...
  <div class="max-w-7xl mx-auto px-6">
    ...
    <div class="grid md:grid-cols-${Math.min(steps.length, 4)} gap-8">
      ${steps.map((s, i) => `<div class="text-center">
        <div class="w-14 h-14 rounded-full mx-auto mb-4 flex items-center justify-center text-2xl font-black" style="background:${t.accent};color:${t.bg}">${i + 1}</div>
  ...
  <div class="max-w-7xl mx-auto px-6">
    ...
    <div class="grid md:grid-cols-${Math.min(items.length, 3)} gap-8">
      ${items.map((item) => `<div class="p-8 rounded-2xl border" style="border-color:${t.accent}20">
        <p class="text-lg italic opacity-80 mb-6">"${esc(item.quote)}"</p>
        <div class="flex items-center gap-3">
          ${item.avatar ? `<img src="${esc(item.avatar)}" alt="" class="w-10 h-10 rounded-full object-cover" />` : `<div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold" style="background:${t.accent}15;color:${t.accent}">${esc(item.author[0] || "?")}</div>`}
          <div>
  ...
  <div class="max-w-7xl mx-auto px-6">
    ...
    <div class="grid md:grid-cols-${Math.min(tiers.length, 3)} gap-8 max-w-5xl mx-auto">
      ${tiers.map((tier) => `<div class="p-8 rounded-2xl border-2 ${tier.highlighted ? "shadow-xl scale-105" : ""}" style="border-color:${tier.highlighted ? t.accent : t.accent + "30"}">
        ...
        <ul class="space-y-3 mb-8">${tier.features.map((f) => `<li class="flex items-center gap-2"><svg class="w-5 h-5 shrink-0" style="color:${t.accent}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>${esc(f)}</li>`).join("")}</ul>
        <a href="#" class="block text-center py-3 rounded-lg font-bold transition ${tier.highlighted ? "hover:opacity-90" : "hover:opacity-80"}" style="${tier.highlighted ? `background:${t.accent};color:${t.bg}` : `border:2px solid ${t.accent};color:${t.accent}`}">${esc(tier.cta || "Get Started")}</a>
      </div>`).join("\n      ")}
    </div>
  </div>
</section>`;
  ...
  stats: (p, t) => {
    ...
    return `<section style="background:${t.accent};color:${t.bg}" class="py-20">
  <div class="max-w-7xl mx-auto px-6">
  ...
  <div class="max-w-3xl mx-auto px-6">
    <h2 class="text-3xl lg:text-4xl font-extrabold text-center mb-16">${title}</h2>
    <div class="space-y-4">
      ${items.map((faq) => `<details class="group rounded-xl border p-6 cursor-pointer" style="border-color:${t.accent}20">
  ...
  <div class="max-w-4xl mx-auto px-6 text-center">
    ...
    <div class="mt-10">
      <a href="${ctaUrl}" class="inline-block px-8 py-4 rounded-lg text-lg font-bold shadow-lg hover:opacity-90 transition" style="background:${t.bg};color:${t.accent}">${ctaText}</a>
    </div>
  </div>
</section>`;
  },
  ...
  footer: (p, t) => {
    ...
    return `<footer style="background:${t.text};color:${t.bg}" class="py-12">
  <div class="max-w-7xl mx-auto px-6">
