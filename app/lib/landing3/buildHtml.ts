...
import { buildThemeCss, buildSingleThemeCss, buildCustomTheme } from "./themes";
 ...
 */
export function buildDeployHtml(sections: Section3[], theme?: string, customColor?: string): string {
  const sorted = [...sections].sort((a, b) => a.order - b.order);
  ...
  // For custom theme, build CSS from the custom color directly (no data-theme needed, inject as :root)
  const { css: baseCss, tailwindConfig } = isCustom
    ? (() => {
        const ct = buildCustomTheme(customColor);
