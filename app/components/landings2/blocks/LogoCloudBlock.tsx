          ...
          <div
            className={
              c.variant === "row"
                ...
                : "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 items-center"
            }
          >
            {logos.map((logo: any, i: number) => (
              <div key={i} className="relative group border border-dashed border-gray-300 rounded-xl p-4 flex flex-col items-center gap-2">
                <button
                  type="button"
                ...
                {logo.imageUrl ? (
                  <img src={logo.imageUrl} alt={logo.alt || ""} className="h-12 object-contain" />
                ) : (
                  <div className="h-12 w-24 bg-gray-100 rounded flex items-center justify-center text-gray-400 text-xs">Logo</div>
                )}
                <input
                  type="text"
                  value={logo.imageUrl || ""}
